...
function wrapAssertions(callbacks) {
	...
	const assertions = {
		...
		snapshot(expected, message) {
			const result = this._test.compareWithSnapshot(expected);
			if (result.pass) {
				pass(this);
			} else if (result.actual) {
				const diff = formatAssertError.formatDescriptorDiff(result.actual, result.expected);
				const values = diff ? [diff] : [
					formatAssertError.formatDescriptorWithLabel('Snapshot:', result.actual),
					formatAssertError.formatDescriptorWithLabel('Must be deeply equal to:', result.expected)
				];
