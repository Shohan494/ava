...
class Runner extends EventEmitter {
	...
	compareTestSnapshot(test, expected) {
		if (!this.snapshots) {
			...
			this.snapshots = snapshot.load(dir, name, this.updateSnapshots);
		}

		return this.snapshots.compare(test.title, expected);
	}
	...
	saveSnapshotState() {
		...
		// TODO: There may be unused snapshot files if no test caused the snapshots
		// to be loaded. Prune them. But not if tests (including hooks!) were skipped.
	}
